...
public class ForceCardUtility
{
    ...
    private const float ForceButtonSize = 46f;

    private const float ForceButtonPointSize = 24f;

    private const float HeaderSize = 32f;

    private const float TextSize = 22f;

    private const float Padding = 3f;

    private const float SpacingOffset = 15f;

    private const float SectionOffset = 8f;

    private const float SkillsColumnHeight = 113f;

    private const float SkillsBoxSize = 18f;

    private const float PowersColumnHeight = 195f;

    private const float PowersColumnWidth = 123f;

    // RimWorld.CharacterCardUtility
    private static Vector2 forceCardSize;

    public static float ColumnSize = 245f;

    private static float skillsColumnDivider = 114f;

    private static float skillsTextWidth = 138f;

    private static bool adjustedForLanguage;
    ...
    private static void adjustForLanguage()
    {
    ...
    private static string alignmentTipString()
    {
        return "PJ_AlignmentDesc".Translate();
    }

    private static string forceXpTipString(CompForceUser compForce, bool sensitive)
    {
        if (!sensitive)
        {
    ...
    private static void AlignmentOnGUI(Rect rect, CompForceUser compForce)
    {
        ...
        TooltipHandler.TipRegion(rect3,
            new TipSignal(alignmentTipString, rect.GetHashCode()));
    ...
    private static void infoPane(Rect inRect, CompForceUser compForce)
    {
    ...
    private static void infoPaneSensitive(Rect inRect, CompForceUser compForce)
    {
        var rectLevel = new Rect(inRect.x, inRect.y, inRect.width * 0.7f, TextSize);
        Text.Font = GameFont.Small;
    ...
    private static void drawLevelBar(Rect rect, CompForceUser compForce, bool sensitive = false)
    {
        ////base.DrawOnGUI(rect, maxThresholdMarkers, customMargin, drawArrows, doTooltip);
        if (rect.height > 70f)
        {
    ...
    private static void skillsPane(Rect inRect, CompForceUser compForce)
    {
    ...
    private static void powersGUIHandler(Rect inRect, CompForceUser compForce, List<ForcePower> forcePowers,
