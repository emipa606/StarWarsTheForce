...
public class DamageWorker_ForcePull : DamageWorker_ForceLeveled
{
    ...
    public override void MasterEffect(Thing target)
    {
        ...
        if (target.def.EverHaulable && target is not Pawn)
        {
            //If it's equippable, equip it right away.
            if (target.def.equipmentType == EquipmentType.Primary || target.def.IsApparel)
            {
                flyingObject = (FlyingObject_Equipable)GenSpawn.Spawn(ThingDef.Named("PJ_PFlyingObject_Equipable"),
                    target.Position, target.Map);
            }
            //Or don't equip it~~
            else
            {
                flyingObject = (FlyingObject)GenSpawn.Spawn(ThingDef.Named("PJ_PFlyingObject"),
                    target.Position, target.Map);
            }
        ...
        if (target is not Pawn pawnTarget)
        {
            return;
        }
