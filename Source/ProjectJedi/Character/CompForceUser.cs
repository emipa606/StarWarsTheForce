...
public class CompForceUser : CompAbilityUser
{
    ...
    {
        get => !IsForceUser ? 0 : ForceData.Level;
    ...
    {
        get => !IsForceUser ? 0 : ForceData.XP;

        set => ForceData.XP = value;
    }

    private float XPLastLevel
    {
        get
        {
            var result = 0f;
    ...
    public int DarksidePoints
    {
        get
        {
            ...
            var result = 0;
            if (ForceData.PowersDark is not { Count: > 0 })
            {
                return result;
            }
    ...
    public int LightsidePoints
    {
        get
        {
            ...
            var result = 0;
            if (ForceData.PowersLight is not { Count: > 0 })
            {
                return result;
            }
    ...
    {
        get => !IsForceUser ? 0 : ForceData.Alignment;

        set
        {
            ForceData.Alignment = Math.Clamp(value, 0f, 1.0f);
            if (ForceUserLevel > 0)
            {
                UpdateAlignment();
            }
    ...
    public ForceAlignmentType ForceAlignmentType
    {
        get
        {
            ...
            return ForceData.Alignment < 0.6 ? ForceAlignmentType.Gray : ForceAlignmentType.Light;
        }
    ...
    // RimWorld.TraitSet
    private void LoseTrait(TraitSet traits, Trait trait)
    {
    ...
    public override void CompTick()
    {
        ...
        {
            LessonAutoActivator.TeachOpportunity(ConceptDef.Named("PJ_TutorForceSensitive"), Pawn,
        ...
        if (ModInfo.TutorLesson_OtherForce)
        {
            ...
            LessonAutoActivator.TeachOpportunity(ConceptDef.Named("PJ_TutorForceAlignment"), Pawn,
                OpportunityType.Important);
        }
    ...
    private void resolveForceTab()
    {
        // PostExposeData();
        // Make the ITab
        var tabs = Pawn.GetInspectTabs();
        var inspectTabBases = tabs as InspectTabBase[] ?? tabs.ToArray();
        if (!inspectTabBases.Any())
        {
            return;
        }

        if (inspectTabBases!.FirstOrDefault(x => x is ITab_Pawn_Force) != null)
        {
            return;
        }
        ...
        {
            Log.Error(
                $"Could not instantiate inspector tab of type {typeof(ITab_Pawn_Force)}: {ex}");
        }
    }

    private void resolveForcePowers()
    {
        ...
        var sithTrait = Pawn.story.traits.GetTrait(ProjectJediDefOf.PJ_SithTrait);
        var grayTrait = Pawn.story.traits.GetTrait(ProjectJediDefOf.PJ_GrayTrait);
        var forceSensitive = Pawn.story.traits.GetTrait(ProjectJediDefOf.PJ_ForceSensitive);
        ...
        if (forceSensitive != null)
        {
            ForceData.Alignment = Rand.Range(0.401f, 0.599f);
        }
    }

    private void resolveForcePool()
    {
